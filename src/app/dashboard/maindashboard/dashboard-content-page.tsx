...
export const DashboardContentPage = () => {
  ...
  const { data: categories, isPending: isEventCategoriesLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const data = superjson.parse(responseText) as {
    ...
    <>
      <ul className="grid max-w-6xl geid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
        {categories.map((category) => (
          <li
            key={category.id}
            className="relative group z-10 transition-all duration-200 hover:-translate-x-0.5"
          >
                ...
                <div className="flex items-center text-sm/6 text-gray-600">
                  ...
                  <span className="ml-1">
                    {category.lastPing && isValid(new Date(category.lastPing))
                      ? formatDistanceToNow(new Date(category.lastPing)) +
                        " ago"
                      : "never"}
                  </span>
                </div>
