...
const EVENT_CATEGORY_VALIDATE = z.object({
  ...
  color: z
    ...
    .regex(/^#[0-9A-F]+$/i, "invalid color format"),
...
interface CreateContentCategoryModal extends PropsWithChildren {
  containerClassName?: string;
}

export const CreateContentCategoryModal = ({
  children,
  containerClassName,
  ...
  const { mutate: createEventCategory } = useMutation({
    mutationFn: async (data: EventCategoryForm) => {
      const response = await axios.post(
        "/api/category/createEventCategory",
        data,
        {
          headers: {
      ...
      <Modal showModal={isOpen} setShowModal={setIsOpen}>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <div>
            ...
            <p className="text-sm/7 text-neutral-650">
              Create a new category to organize the event
            </p>
          </div>
          ...
          <div>
            ...
            {COLOR_OPTIONS.map((premadeColor) => (
              <button
                ...
                className={cn(
                  `bg-[${premadeColor}]`,
                  "size-10 rounded-full ring-2 ring-offset-2 transition-all ml-2",
                  color === premadeColor
                    ? "ring-cyan-700 scale-110"
          ...
          <div>
            ...
            {EMOJI_OPTIONS.map(({ emoji, label }) => (
              <button
                ...
                type="button"
                className={cn(
                  "size-10 rounded-md text-xl justify-center items-center transition-all mt-2 ml-2",
                  selectedEmoji === emoji
